...
<head>
    ...
    <title>{{ __('frontend.auth.password_reset_new_title') }} - {{ __('frontend.site_name') }}</title>
    ...
    <div class="w-full max-w-md">
        <div class="bg-white rounded-lg shadow-lg p-8">
            <div class="text-center mb-8">
                ...
                <h2 class="text-2xl font-bold text-gray-800">{{ __('frontend.auth.password_reset_new_title') }}</h2>
                <p class="text-sm text-gray-600 mt-2">{{ __('frontend.auth.password_reset_new_message') }}</p>
            </div>
            ...
            <form action="{{ route('password.reset') }}" method="POST" class="space-y-4">
                @csrf
                <div>
                    <label for="password" class="block text-sm font-semibold text-gray-700 mb-2">{{ __('frontend.profile.new_password') }}</label>
                    <div class="relative">
                ...
                <div>
                    <label for="password_confirmation" class="block text-sm font-semibold text-gray-700 mb-2">{{ __('frontend.auth.password_confirmation') }}</label>
                    <div class="relative">
                        <input type="password" id="password_confirmation" name="password_confirmation" required
                               class="w-full px-4 py-3 border border-gray-300 rounded-lg input-field focus:ring-2 focus:ring-secondary pr-12">
                        <button type="button" onclick="togglePassword('password_confirmation', 'passwordConfirmToggle')"
                                class="absolute {{ app()->getLocale() === 'ar' ? 'left-4' : 'right-4' }} top-1/2 transform -translate-y-1/2 text-gray-500 hover:text-primary">
            ...
            <div class="mt-6 text-center">
                <a href="{{ route('login') }}" class="text-sm text-primary hover:text-secondary font-semibold transition">
                    <i class="fas fa-arrow-right ml-1"></i> {{ __('frontend.auth.login') }}
                </a>
            </div>
        </div>
    </div>
